...
temp_graph = st.empty() #this is the line chart of the temp on the dashboard
...
while True:
    ...
    fig = px.line(data, x="ts", y="tempF", title="Temperature Over Time")
    temp_graph.plotly_chart(fig)

    # Create a line chart for humidity
    fig = px.line(data, x="ts", y="humidity", title="Humidity Over Time")
    humid_graph.plotly_chart(fig)
